...
class VersionRange(VersionConstraint):
    ...
    def allows_all(self, other):  # type: (VersionConstraint) -> bool
        ...
        if isinstance(other, VersionUnion):
            return all([self.allows_all(constraint)
                        for constraint in other.ranges])

        if isinstance(other, VersionRange):
            return not other.allows_lower(self) \
                and not other.allows_higher(self)
    ...
    def allows_any(self, other):  # type: (VersionConstraint) -> bool
        ...
        if isinstance(other, VersionRange):
            return not other.is_strictly_lower(self) \
                and not other.is_strictly_higher(self)
    ...
    def intersect(self,
        ...
        if not isinstance(other, VersionRange):
            raise ValueError(
                "Unknown VersionConstraint type {}.".format(other))
        ...
        return VersionRange(
            ...
            intersect_include_min, intersect_include_max
        )
    ...
    def union(self, other):  # type: (VersionConstraint) -> VersionConstraint
        ...
        if isinstance(other, Version):
            ...
            if other == self.min:
                return VersionRange(
                    self.min, self.max,
                    include_min=True, include_max=self.include_max
                )

            if other == self.max:
                return VersionRange(
                    self.min, self.max,
                    include_min=self.include_min, include_max=True
                )

            return VersionUnion.of(self, other)

        if isinstance(other, VersionRange):
            # If the two ranges don't overlap,
            # we won't be able to create a single
            # VersionRange for both of them.
            edges_touch = (
                self.max == other.min and (
                    self.include_max or other.include_min)
            ) or (self.min == other.max and (
    ...
    def difference(self,
        ...
        if isinstance(other, Version):
            ...
            if not self.allows_lower(other):
                ...
                before = VersionRange(
                    self.min, other.min,
                    self.include_min, not other.include_min
                )
            ...
            if not self.allows_higher(other):
                ...
                after = VersionRange(
                    other.max, self.max,
                    not other.include_max, self.include_max
                )
            ...
            for range in other.ranges:
                ...
                # If we reach a range strictly higher than [current],
                # no more ranges
                ...
                if difference.is_empty():
                    ...
                    # If [range] split [current] in half,
    ...
    def __str__(self):
        ...
        if self.max is not None:
            ...
            text += "{}{}".format("<=" if self.include_max else "<",
                                  self.max.text)
