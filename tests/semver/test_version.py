...
from roo.semver import EmptyConstraint
from roo.semver import Version
from roo.semver import VersionRange
from roo.semver.exceptions import ParseVersionError
...
def test_allows_any():
    ...
    assert v.allows_any(VersionRange(
        Version.parse("1.1.4"), Version.parse("1.2.4")))
    assert v.allows_any(VersionRange())
...
def test_intersect():
    ...
    assert (
        v.intersect(VersionRange(Version.parse(
            "1.1.4"), Version.parse("1.2.4"))) == v
    )
    assert (
        Version.parse("1.1.4")
...
def test_union():
    ...
    result = v.union(VersionRange(
        Version.parse("0.0.3"), Version.parse("1.1.4")))
    assert result.allows(v)
...
def test_difference():
    ...
    assert (
        v.difference(VersionRange(Version.parse(
            "1.4.0"), Version.parse("3.0.0"))) == v
    )
